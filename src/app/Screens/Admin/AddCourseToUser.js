...
import { addCoursesToStudent, getAllCourses, getAllOtherCourses } from '../../../core/modules/course/api';
        ...
        <SafeAreaView className="flex-1 justify-start bg-white">
            ...
            >
                <View className="justify-between items-start space-y-2">
                    ...
                    <Text style={{ fontFamily: 'Poppins_600SemiBold' }} className="text-2xl">Vakken toevoegen</Text>
                        ...
                        <Controller
                            ...
                            render={({ field: { onChange, onBlur, value } }) => (
                                <View className="mt-12">
                                    <Text style={{ fontFamily: 'Poppins_500Medium' }} className="text-sm mb-2">Vakken</Text>
                                    <MultiSelect
                                        label="Vakken"
                                        data={courses}
                                        ...
                                        inputSearchStyle={{ fontFamily: 'Poppins_400Regular', fontSize: 14, paddingTop: 1, color: '#0F172A', borderRadius: 10, paddingHorizontal: 10, backgroundColor: '#f8fafc' }}
                                        renderSelectedItem={(item, unSelect) => (
                                            <TouchableOpacity onPress={unSelect} className="mt-2 mr-3">
                                                <View className="flex-row space-x-2 items-center rounded-full bg-slate-100 px-4 py-2">
                                                    <Text style={{ fontFamily: 'Poppins_400Regular' }} className="text-base">{item.name}</Text>
                                                        <XMarkIcon size={16} color="#0F172A" />
                                                </View>
                                            </TouchableOpacity>
                                        )}
                                    />
